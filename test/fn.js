import katakanize from '../lib/fn/katakanize';
...
describe('Jaco Static Methods', () => {
	...
	it('ひらがなをカタカナに変換', () => {
		...
		katakanize(test).should.equal(mean);
	});
	it('カタカナをひらがなに変換', () => {
		const test = [
		...
		hiraganize(test).should.equal(mean);
	});
	it('カタカナをひらがなに変換(濁点・半濁点を結合文字に変換)', () => {
		const test = [
			'ｦｧｨｩｪｫｬｭｮｯｰｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾍﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾛﾜﾝｶﾞｷﾞｸﾞｹﾞ',
			'ｺﾞｻﾞｼﾞｽﾞｾﾞｿﾞﾀﾞﾁﾞﾂﾞﾃﾞﾄﾞﾊﾞﾋﾞﾌﾞﾍﾞﾎﾞﾊﾟﾋﾟﾌﾟﾍﾟﾎﾟﾜﾞｲﾞｳﾞｴﾞｦﾞヲァィゥェォャ',
			'ュョッーアイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホ',
		...
		hiraganize(test, true).should.equal(mean);
	});
	it('カタカナをひらがなに変換', () => {
		hiraganize('ガガガガガガガガ').should.equal('がががががががが');
	});
	it('カタカナをひらがなに変換', () => {
		hiraganize('ガガガガガガガガ').should.equal('がががががががが');
	});

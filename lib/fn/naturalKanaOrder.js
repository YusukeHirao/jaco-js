"use strict";
const isOnlyHiragana_1 = require("./isOnlyHiragana");
...
const toPhoeticKana_1 = require("./toPhoeticKana");
/**
...
function default_1(a, b) {
    ...
    const phoneticA = toPhoeticKana_1.default(toNarrow_1.default(a));
    const phoneticB = toPhoeticKana_1.default(toNarrow_1.default(b));
    const unvoicedA = removeVoicedMarks_1.default(phoneticA, true);
    const unvoicedB = removeVoicedMarks_1.default(phoneticB, true);
    ...
    if (codeA < codeB) {
        ...
        for (let i = 0; i < l; i++) {
            // ひらがな・カタカナで比較
            const codePointA = isOnlyHiragana_1.default(a[i]) ? 0 : 1;
            const codePointB = isOnlyHiragana_1.default(b[i]) ? 0 : 1;
...
function _convertNaturalKanaOrderNumberPhase2(str) {
    ...
    const result = str
        .replace('ー', '0')
        .replace(/[ぁぃぅぇぉっゃゅょゎァィゥェォヵㇰヶㇱㇲッㇳㇴㇵㇶㇷㇸㇹㇺャュョㇻㇼㇽㇾㇿヮ]/, ($0) => {

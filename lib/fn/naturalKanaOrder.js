...
function default_1(a, b) {
    ...
    const _a = toPhoeticKana_1.default(toNarrow_1.default(a));
    const _b = toPhoeticKana_1.default(toNarrow_1.default(b));
    let _tmpA; // tempString
    let _tmpB; // tempString
    const phoneticA = _a.toString();
    const phoneticB = _b.toString();
    const unvoicedA = removeVoicedMarks_1.default(_a, true);
    const unvoicedB = removeVoicedMarks_1.default(_b, true);
    ...
    if (codeA < codeB) {
        ...
        for (let i = 0; i < l; i++) {
            // ひらがな・カタカナで比較
            _tmpA = isOnlyHiragana_1.default(a[i]) ? '0' : '1';
            _tmpB = isOnlyHiragana_1.default(b[i]) ? '0' : '1';
            if (_tmpA < _tmpB) {
                return -1;
            }
            else if (_tmpA > _tmpB) {

...
const hiraganaOnly_1 = require("./hiraganaOnly");
/**
...
function default_1(a, b) {
    ...
    const _a = new jaco_1.default(a).toNarrow().toPhoeticKana();
    const _b = new jaco_1.default(b).toNarrow().toPhoeticKana();
    ...
    if (codeA < codeB) {
        ...
        for (let i = 0; i < l; i++) {
            // ひらがな・カタカナで比較
            _tmpA = hiraganaOnly_1.default(a[i]) ? '0' : '1';
            _tmpB = hiraganaOnly_1.default(b[i]) ? '0' : '1';
            if (_tmpA < _tmpB) {
                return -1;
            }
            else if (_tmpA > _tmpB) {
